...
A small Python library for discovering letter-level correspondences between two parallel orthographies. It alternates iterative φ-coefficient scoring with Needleman-Wunsch-style alignment, and is deliberately agnostic about which scripts you're aligning, what your CSV columns are called, or how you normalize each side.

This repository ships three Python distributions:

1. **`scriptalign`** — the domain-neutral library, at the root.
2. **`coptic-arabic`** — alignment of Coptic-script Arabic against vocalized Arabic, expressed as a client of `scriptalign`. The φ-scoring + DP method itself originates with Christian Casey's analysis of this corpus.
3. **`arabic-armenian`** — alignment of 27 Arabic philosophical terms against their Armenian-script transcriptions, also a client of `scriptalign`.

The two example clients are intended to demonstrate that the alignment runs without interference and reproduces the results discussed in the corresponding papers. The alignment and the evidence lead; analysis comes after. All results should be confirmed by repeating the analysis.

## Install

Install whichever distributions you need:
...
All corpus-specific knowledge lives under `examples/` — see `examples/coptic_arabic/` and `examples/arabic_armenian/` for two worked-out clients.

## Example: Coptic-Arabic
...
Input is in `parallel_texts.csv` at the repo root. The corpus is drawn from:
...
27-pair corpus bundled with the `arabic-armenian` package. Source: <https://kieranmeinhardt.de/language/arabic-armenian/>.
